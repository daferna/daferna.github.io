...
Another way is to have a one-time code generator like Google Authenticator/Auth/Duo.
...
on my phone for on the go access. The file, of course, is encrypted, so despite the fact that Dropbox has been hackedmultiple times, it should be hard to crack through a brute-force password attack.
...
Conside. [being able to pay for stuff without removing the card from your wallet a net *benefit*, not hinderance.
